...
#     文章的自身特征， category_id表示这文章的类型， created_at_ts表示文章建立的时间， 这个关系着文章的时效性， words_count是文章的字数，
# 一般字数太长我们不太喜欢点击, 也不排除有人就喜欢读长文。
...
# 但是我们需要先基于召回的结果， 构造一些特征，然后制作标签，形成一个监督学习的数据集。 构造监督数据集的思路， 根据召回结果，
# 我们会得到一个{user_id: [可能点击的文章列表]}形式的字典。 那么我们就可以对于每个用户， 每篇可能点击的文章构造一个监督测试集，
# 比如对于用户user1， 假设得到的他的召回列表{user1: [item1, item2, item3]}， 我们就可以得到三行数据(user1, item1), (user1, item2),
# (user1, item3)的形式， 这就是监督测试集时候的前两列特征。
# 构造特征的思路是这样， 我们知道每个用户的点击文章是与其历史点击的文章信息是有很大关联的， 比如同一个主题， 相似等等。
# 所以特征构造这块很重要的一系列特征是要结合用户的历史点击文章信息。我们已经得到了每个用户及点击候选文章的两列的一个数据集， 而我们的目的是要预测最后一次点击的文章，
# 比较自然的一个思路就是和其最后几次点击的文章产生关系， 这样既考虑了其历史点击文章信息， 又得离最后一次点击较近，因为新闻很大的一个特点就是注重时效性。
...
# 读取各种Embedding
    ...
    # 2.使用模型的时候有一些默认值，可以通过在Jupyter里面通过Word2Vec查看
    # 下面是个简单的测试样例：
    from gensim.models import Word2Vec
    doc = [['30760', '157507'],
           ['289197', '63746'],
...
# 下采样的目的一方面缓解了正负样本比例的问题，另一方面也减小了我们做排序特征的压力，我们在做负采样的时候又有哪些东西是需要注意的呢？
    ...
    def recall_dict_2_df(recall_list_dict):
        ...
        return recall_list_df  # 召回和排序负样本选择的区别 召回时负样本是从没交互的数据中选择的  而排序的负样本是未召回
    ...
    def neg_sample_recall_data(recall_items_df, sample_rate=0.001):
        ...
        def neg_sample_func(group_df):
            ...
            sample_num = min(sample_num, 5)  # 保证最多不超过5个，这里可以根据实际情况进行选择  最多不超过1：5
...
# 这里我们只提供其中一种，我们写一个函数， 得到可以衡量用户活跃度的特征，逻辑如下：
        ...
        # 3.注意，上面两两点击文章的时间间隔均值， 会出现如果用户只点击了一次的情况，这时候时间间隔均值那里会出现空值，对于这种情况最后特征那里给个大数进行区分（不活跃）
...
# 用户的系列习惯
    ...
    # * 用户的时间习惯： 根据其点击过得历史文章的时间来做一个统计（这个感觉最好是把时间戳里的时间特征的h特征提出来，看看用户习惯一天的啥时候点击文章），
# 但这里先用转换的时间吧， 求个均值
    ...
    def device_fea(all_data, cols):
        ...
        # 用众数来表示每个用户的设备信息
        user_device_info = user_device_info.groupby('user_id').agg(lambda x: x.value_counts().index[0]).reset_index() # counts降序
    ...
    def user_time_hob_fea(all_data, cols):
        ...
        # 先把时间戳进行归一化
        mm = MinMaxScaler()
        user_time_hob_info['click_timestamp'] = mm.fit_transform(user_time_hob_info[['click_timestamp']]) # 所有用户的平均观看时间点
        user_time_hob_info['created_at_ts'] = mm.fit_transform(user_time_hob_info[['created_at_ts']]) # 所有用户的平均创作时间点
    ...
    if val_user_item_feats_df is not None:
        val_user_item_feats_df = val_user_item_feats_df.merge(articles, left_on='click_article_id',
                                                              right_on='article_id')
    else:
        val_user_item_feats_df = None
    tst_user_item_feats_df = tst_user_item_feats_df.merge(articles, left_on='click_article_id', right_on='article_id')

    # 召回文章的主题是否在用户的爱好里面 这个特征可以帮助模型更好地理解用户对不同主题的兴趣程度
...
# 总结
    # 特征工程和数据清洗转换是比赛中至关重要的一块， 因为数据和特征决定了机器学习的上限，而算法和模型只是逼近这个上限而已，
    # 所以特征工程的好坏往往决定着最后的结果，特征工程可以一步增强数据的表达能力，通过构造新特征，我们可以挖掘出数据的更多信息，使得数据的表达能力进一步放大。
    # 在本节内容中，我们主要是先通过制作特征和标签把预测问题转成了监督学习问题，然后围绕着用户画像和文章画像进行一系列特征的制作，
    # 此外，为了保证正负样本的数据均衡，我们还学习了负采样技术等。当然本节内容只是对构造特征提供了一些思路，也请学习者们在学习过程中开启头脑风暴，
    # 尝试更多的构造特征的方法，也欢迎我们一块探讨和交流。