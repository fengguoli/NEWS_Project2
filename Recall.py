...
metric_recall = False
# 评估不用全量数据的理由： 全量数据量大评估效率低 且 数据分布不均匀导致评估结果偏向高频用户或物品

# 读取数据
    # 在一般的推荐系统比赛中读取数据部分主要分为三种模式， 不同的模式对应的不同的数据集：
    # 1. Debug模式： 这个的目的是帮助我们基于数据先搭建一个简易的baseline并跑通， 保证写的baseline代码没有什么问题。
    #     由于推荐比赛的数据往往非常巨大， 如果一上来直接采用全部的数据进行分析，搭建baseline框架， 往往会带来时间和设备上的损耗，
    #     所以这时候我们往往需要从海量数据的训练集中随机抽取一部分样本来进行调试(train_click_log_sample)， 先跑通一个baseline。
    # 2. 线下验证模式： 这个的目的是帮助我们在线下基于已有的训练集数据， 来选择好合适的模型和一些超参数。 所以我们这一块只需要加载整个训练集(train_click_log)， 然后把整个训练集再分成训练集和验证集。
    #     训练集是模型的训练数据， 验证集部分帮助我们调整模型的参数和其他的一些超参数。
    # 3. 线上模式： 我们用debug模式搭建起一个推荐系统比赛的baseline， 用线下验证模式选择好了模型和一些超参数， 这一部分就是真正的对于给定的测试集进行预测， 提交到线上，
    #     所以这一块使用的训练数据集是全量的数据集(train_click_log+test_click_log)
    ...
    def get_item_emb_dict(data_path):
        ...
        item_emb_cols = [x for x in item_emb_df.columns if 'emb' in x]  # 包含emb_0 - emb_249
        item_emb_np = np.ascontiguousarray(item_emb_df[item_emb_cols])
        # 进行归一化
        item_emb_np = item_emb_np / np.linalg.norm(item_emb_np, axis=1, keepdims=True) # 向量 / 范数
        item_emb_dict = dict(zip(item_emb_df['article_id'], item_emb_np))
        pickle.dump(item_emb_dict, open(save_path / 'item_content_emb.pkl', 'wb'))  # pickle 模块可以序列化和反序列化几乎任何 Python 对象
...
# 工具函数
    ...
    def get_hist_and_last_click(all_click):
        ...
        click_last_df = all_click.groupby('user_id').tail(1)  # .tail(1) 返回最后一行 经过排序后最后一行就是最近的一次 值最大的一个
        ...
        def hist_func(user_df):
            if len(user_df) == 1:
                ...
                return user_df[:-1]
        # hist_func：一个辅助函数，用于处理每个用户的历史点击记录
        # 如果用户只有一个点击记录，直接返回该记录（避免历史记录为空）
        # 如果用户有多个点击记录，返回除最后一次点击外的所有记录
    ...
    # 召回效果评估
    # 为什么需要评估： 做完了召回有时候也需要对当前的召回方法或者参数进行调整以达到更好的召回效果，因为召回的结果决定了最终排序的上限，下面也会提供一个召回评估的方法
    ...
    def metrics_recall(user_recall_items_dict, trn_last_click_df, topk=5):
        ...
        for k in range(10, topk+1, 10):
            ...
            for user, item_list in user_recall_items_dict.items():
                # 获取前k个召回的结果
                tmp_recall_items = [x[0] for x in user_recall_items_dict[user][:k]]  # 值是元组 所以x【0】是ID
                if last_click_item_dict[user] in set(tmp_recall_items):  # 改进点 ： 可以关注lastitem在recall中的位置信息 加权
                    hit_num += 1  # hit_num是累加的  计算的是全体user的命中率
            hit_rate = round(hit_num * 1.0 / user_num, 5)  # 五位小数
...
# 计算相似性矩阵
        ...
        for user, item_time_list in tqdm(user_item_time_dict.items(), disable=not logger.isEnabledFor(logging.DEBUG)):
            ...
            for loc1, (i, i_click_time) in enumerate(item_time_list):
                ...
                for loc2, (j, j_click_time) in enumerate(item_time_list):
                    ...
                    i2i_sim[i][j] += loc_weight * click_time_weight * created_time_weight / math.log(len(item_time_list) + 1) # 防止分母为0
                    # 归一化 用于确保不同特征或计算结果在相同的尺度上，从而提高模型的准确性和稳定性
        i2i_sim_ = i2i_sim.copy()
        for i, related_items in i2i_sim.items():
            for j, wij in related_items.items():
                i2i_sim_[i][j] = wij / math.sqrt(item_cnt[i] * item_cnt[j])
                # 流行物品（点击次数多的物品）通常会与更多其他物品共现，导致其相似度权重偏高。通过归一化，可以调整这种偏差，使相似度权重不受物品流行度的影响。
        # 将得到的相似性矩阵保存到本地
    ...
    def get_user_activate_degree_dict(all_click_df):
        ...
        return user_activate_degree_dict  # 点击次数代表活跃度
    ...
    # 计算物品相似度 用以user为键的字典 而 计算用户相似度用以item为键的字典
    ...
    # faiss工具包一般使用在推荐系统中的向量召回部分。在做向量召回的时候要么是u2u,u2i或者i2i，这里的u和i指的是user和item.我们知道在实际的场景中user和item的数量都是海量的，
    # 我们最容易想到的基于向量相似度的召回就是使用两层循环遍历user列表或者item列表计算两个向量的相似度，但是这样做在面对海量数据是不切实际的，faiss就是用来加速计算某个查询向量最相似的topk个索引向量。
    # faiss查询的原理：
    # faiss使用了PCA和PQ(Product quantization乘积量化)两种技术进行向量压缩和编码，当然还使用了其他的技术进行优化，但是PCA和PQ是其中最核心部分。
    # PCA：先求样本集的协方差矩阵 取最大的n个特征值对应的特征向量拼成矩阵W  做特征变换 Y=WX 达到降维的目的
    # 推导过程： 两种构造目标函数的过程  一种是求新恢复向量与原向量的差的范数最小（距离足够近） 一种是求样本的投影方差和最大（尽可能分开） 两种优化目标得到相同的最优解
    # OQ： 将原始的高维向量空间分成多个子空间。假设原始向量是 D 维的，将其分为 M 个子空间，每个子空间包含 D/M 维。
    # 对每个子空间，使用聚类算法（如 k-means）生成一个码本（codebook），码本包含 K 个聚类中心。每个子向量被映射到其最近的聚类中心，并用聚类中心的索引来表示
    # 最终，原始向量被表示为 M 个索引的序列，这些索引称为 PQ 编码。这种表示方式大大减少了存储需求
    # 预计算码本之间的距离  这样可以通过查表的方式计算相似度
    # 在 PQ 编码中，通过查表方式计算向量相似度时，所有向量之间的距离确实被固化了，距离的取值范围是一个确定的有限集合。
    # 这是因为 PQ 编码将每个向量分解为多个子向量，并将每个子向量量化到一个预定义的码本中的某个聚类中心。这种方法虽然牺牲了一定的精度，但显著提高了计算效率，特别适用于大规模数据集和实时查询场景。
    # 距离的取值范围是一个固定的有限集合，这可能导致某些情况下无法精确区分相似度。例如，两个向量的实际距离可能非常接近，但通过查表方式计算的距离可能相同，从而导致无法区分它们的相似度。
    # 选择合适的码本大小 K 是关键。较大的码本可以减少量化误差，但会增加存储和计算成本。较小的码本可以提高存储和计算效率，但会增加量化误差。
    # 向量检索相似度计算
    ...
    def embdding_sim(click_df, item_emb_df, save_path, topk):
        ...
        # 向量进行单位化
        item_emb_np = item_emb_np / np.linalg.norm(item_emb_np, axis=1, keepdims=True)  # 归一化 使每个向量长度为1
        # 建立faiss索引
        item_index = faiss.IndexFlatIP(item_emb_np.shape[1])  # 直接内积检索 并没有进行再编码
...
# 召回
    # 这个就是我们开篇提到的那个问题， 面的36万篇文章， 20多万用户的推荐， 我们又有哪些策略来缩减问题的规模？ 我们就可以再召回阶段筛选出用户对于点击文章的候选集合， 从而降低问题的规模。
    # 召回常用的策略： * Youtube DNN 召回 * 基于文章的召回 * 文章的协同过滤 * 基于文章embedding的召回 * 基于用户的召回 * 用户的协同过滤 * 用户embedding
    # 上面的各种召回方式一部分在基于用户已经看得文章的基础上去召回与这些文章相似的一些文章， 而这个相似性的计算方式不同， 就得到了不同的召回方式， 比如文章的协同过滤， 文章内容的embedding等。
    # 还有一部分是根据用户的相似性进行推荐，对于某用户推荐与其相似的其他用户看过的文章，比如用户的协同过滤和用户embedding。
    ...
    def gen_data_set(data, negsample=0):
        ...
        for reviewerID, hist in tqdm(data.groupby('user_id'), disable=not logger.isEnabledFor(logging.DEBUG)):
            pos_list = hist['click_article_id'].tolist()  # 点击过的视为正样本
            if negsample > 0:
                candidate_set = list(set(item_ids) - set(pos_list))   # 用户没看过的文章里面随机选择负样本
            ...
            for i in range(1, len(pos_list)):
                hist = pos_list[:i]  # 前i个元素作为历史行为 第i个元素作为目标行为
                if i != len(pos_list) - 1:
                    train_set.append((reviewerID, hist[::-1], pos_list[i], 1, len(hist[::-1])))  # 正样本 [user_id, his_item, pos_item, label, len(his_item)]
                    # 可能是为了让模型更关注近期行为（最新的行为在前面）
                    for negi in range(negsample):  # 正样本取一个 负样本要取negsample个
        ...
        random.shuffle(test_set)  # 打乱训练集和测试集，以提高模型的泛化能力
    ...
    def gen_model_input(train_set,user_profile,seq_max_len):
        ...
        train_seq_pad = pad_sequences(train_seq, maxlen=seq_max_len, padding='post', truncating='post', value=0)  # 末尾填充和末尾截断
    ...
    # 为什么这个召回模型叫YouTubeDNN：
            # 于我而言，迁移一个模型不需要完全迁移，只需要抓住该模型的独特之处，并且这个独特之处可以解决我的问题，我就可以将这部分独特之处迁移到我的问题中，就可以冠名
            # 而YouTubeDNN的独特之处我觉得在于：
                # 处理时间序列时避免信息泄露（不可以进行随机留一构造训练集，必须选末尾）
                # Fassis快速查找
                # 还有样本平衡固定每个用户的历史行为序列长度 防止高度活跃用户对模型训练过程产生不成比例的影响
                # 滑动窗口技术：对于用户行为较少的情况，可以通过滑动窗口的方式增加训练样本数量
    ...
    # 这里在召回的时候，也是用了关联规则的方式：
        ...
        # 考虑文章内容相似度权重(使用Embedding计算相似文章相似度，但是这里需要注意，在Embedding的时候并没有计算所有商品两两之间的相似度，
            # 所以相似的文章与历史点击文章不存在相似度，需要做特殊处理)
    # 基于商品的召回i2i
    def item_based_recommend(user_id, user_item_time_dict, i2i_sim, sim_item_topk, recall_item_num, item_topk_click, item_created_time_dict, emb_i2i_sim):
        ...
        for loc, (i, click_time) in enumerate(user_hist_items):
            for j, wij in sorted(i2i_sim[i].items(), key=lambda x: x[1], reverse=True)[:sim_item_topk]:
                ...
                item_rank[j] += created_time_weight * loc_weight * content_weight * wij # 内容相似性用到了emb_i2i_sim增加多样性和准确性
        # 不足10个，用热门商品补全
    ...
    user_recall_items_dict = defaultdict(dict)
    user_item_time_dict = get_user_item_time(trn_hist_click_df)
    i2i_sim = pickle.load(open(save_path / 'emb_i2i_sim.pkl','rb')) # 不同之处
    sim_item_topk = 20
    recall_item_num = 10
    item_topk_click = get_item_topk_click(trn_hist_click_df, k=50)
    for user in tqdm(trn_hist_click_df['user_id'].unique(), disable=not logger.isEnabledFor(logging.DEBUG)):
    ...
    # userCF召回
    # 基于用户协同过滤，核心思想是给用户推荐与其相似的用户历史点击文章，因为这里涉及到了相似用户的历史文章，
    # 这里仍然可以加上一些关联规则来给用户可能点击的文章进行加权，这里使用的关联规则主要是考虑相似用户的历史点击文章与被推荐用户历史点击商品的关系权重，
    # 而这里的关系就可以直接借鉴基于物品的协同过滤相似的做法，只不过这里是对被推荐物品关系的一个累加的过程，下面是使用的一些关系权重，及相关的代码：
    ...
    def user_based_recommend(user_id, user_item_time_dict, u2u_sim, sim_user_topk, recall_item_num,
        ...
        return items_rank
    # 从用户相似度矩阵里面找 与i相似的用户j j的交互物品如果不在i的交互物品中就添加到候选集合 如果遍历完j 得到的候选物品不足就用热度物品补充
    # 计算候选物品与用户i的相似度并排序 按序返回topk
    ...
    # 下面使用了YoutubeDNN过程中产生的user embedding来进行向量检索每个user最相似的topk个user，在使用这里得到的u2u的相似性矩阵，
    # 使用usercf进行召回，具体代码如下
...
# 冷启动问题
    ...
    # (对于我们场景就是，测试集中的用户是否在测试集对应的log数据中出现过，如果没有出现过，那么可以认为该用户是冷启动用户。但是有时候并没有这么严格，
    # 我们也可以自己设定某些指标来判别哪些用户是冷启动用户，比如通过使用时长，点击率，留存率等等)
    # 系统冷启动：就是对于一个平台刚上线，还没有任何的相关历史数据，此时就是系统冷启动，其实也就是前面两种的一个综合。
    # 当前场景下冷启动问题的分析：
    # 对当前的数据进行分析会发现，日志中所有出现过的点击文章只有3w多个，而整个文章库中却有30多万，那么测试集中的用户最后一次点击是否会点击没有出现在日志中的文章呢？
    # 如果存在这种情况，说明用户点击的文章之前没有任何的交互信息，这也就是我们所说的文章冷启动。通过数据分析还可以发现，测试集用户只有一次点击的数据占得比例还不少，
    # 其实仅仅通过用户的一次点击就给用户推荐文章使用模型的方式也是比较难的，这里其实也可以考虑用户冷启动的问题，但是这里只给出物品冷启动的一些解决方案及代码，
    # 关于用户冷启动的话提一些可行性的做法。
    # 文章冷启动(没有冷启动的探索问题) 其实我们这里不是为了做文章的冷启动而做冷启动，而是猜测用户可能会点击一些没有在log数据中出现的文章，
    # 我们要做的就是如何从将近27万的文章中选择一些文章作为用户冷启动的文章，这里其实也可以看成是一种召回策略，我们这里就采用简单的比较好理解的
    # 基于规则的召回策略来获取用户可能点击的未出现在log数据中的文章。 现在的问题变成了：如何给每个用户考虑从27万个商品中获取一小部分商品？
    # 随机选一些可能是一种方案。下面给出一些参考的方案。
    #     首先基于Embedding召回一部分与用户历史相似的文章
    #     从基于Embedding召回的文章中通过一些规则过滤掉一些文章，使得留下的文章用户更可能点击。
    # 我们这里的规则，可以是，留下那些与用户历史点击文章主题相同的文章，或者字数相差不大的文章。并且留下的文章尽量是与测试集用户最后一次点击时间更接近的文章，
    # 或者是当天的文章也行。
    #     用户冷启动 这里对测试集中的用户点击数据进行分析会发现，测试集中有百分之20的用户只有一次点击，那么这些点击特别少的用户的召回是不是可以单独做一些策略上的补充呢？
    # 或者是在排序后直接基于规则加上一些文章呢？这些都可以去尝试，这里没有提供具体的做法。
    # 注意：
    # 这里看似和基于embedding计算的item之间相似度然后做itemcf是一致的，但是现在我们的目的不一样，我们这里的目的是找到相似的向量，
    # 并且还没有出现在log日志中的商品，再加上一些其他的冷启动的策略，这里需要找回的数量会偏多一点，不然被筛选完之后可能都没有文章了
    ...
    def combine_recall_results(user_multi_recall_dict, weight_dict=None, topk=25):
        ...
        for method, user_recall_items in tqdm(user_multi_recall_dict.items(), disable=not logger.isEnabledFor(logging.DEBUG)):
            ...
            for user_id, sorted_item_list in user_recall_items.items(): # 进行归一化
                user_recall_items[user_id] = norm_user_recall_items_sim(sorted_item_list) # 相似性不同度量方式得分归一化
            for user_id, sorted_item_list in user_recall_items.items():
                # print('user_id')
                final_recall_items_dict.setdefault(user_id, {})
                for item, score in sorted_item_list:
                    final_recall_items_dict[user_id].setdefault(item, 0)
                    final_recall_items_dict[user_id][item] += recall_method_weight * score # 每一路召回中的相似性分数累加
...
#       youtubednn召回
#       冷启动召回
#     对于上述实现的召回策略其实都不是最优的结果，我们只是做了个简单的尝试，其中还有很多地方可以优化，包括已经实现的这些召回策略的参数或者新加一些，
#     修改一些关联规则都可以。当然还可以尝试更多的召回策略，比如对新闻进行热度召回等等。